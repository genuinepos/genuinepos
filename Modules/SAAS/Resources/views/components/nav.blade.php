    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse d-flex align-items-center justify-content-between" id="navbarSupportedContent">
            <ul class="navbar-nav mb-2 mb-lg-0">
                <li class="nav-item">
                    @auth
                    <a class="nav-link active" aria-current="page" href="{{ route('saas.dashboard') }}">{{ __('Dashboard') }}</a>
                    @else
                    <a class="nav-link active" aria-current="page" href="{{ route('saas.welcome-page') }}">{{__('Home') }}</a>
                    @endauth
                </li>
                @auth
            ...
            <ul class="navbar-nav  mb-2 mb-lg-0">
                ...
                <li class="nav-item dropdown  float-end">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="navbarDropdownSetting">
                        @can('profile_edit')
                        <li><a class="dropdown-item" href="{{ route('saas.profile.edit', auth()->user()->id) }}">{{ __('Edit Profile') }}</a></li>
                        @endcan
                        <li><hr class="dropdown-divider"></li>
                        <li  onclick="handleLogout()"><a class="dropdown-item" href="#" >{{ __('Logout') }}</a></li>
                    </ul>
                </li>
                @endauth
            </ul>
