<x-saas::admin-layout title="Upgrade Plan">
    @push('css')
        <style>
            ...
            .revel-table .table-responsive tr th {
                padding: 8px 11px;
            }
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <div class="tab-contents">
                                        <form id="upgrade_plan_form" action="{{ route('saas.tenants.upgrade.plan.confirm', $tenant->id) }}" method="POST">
                                            @csrf
                                            <input type="hidden" name="plan_id" id="plan_id" value="{{ $plan->id }}" />
                                            @include('saas::tenants.upgrade_plan.upgrade_plan_from_real_plan.partials.view_partials.cart_table')
                                        </form>
                                    </div>
                                </div>
                            </div>
    ...
    @push('js')
        @include('saas::tenants.upgrade_plan.upgrade_plan_from_real_plan.partials.js_partials.js')
    @endpush
</x-saas::admin-layout>
