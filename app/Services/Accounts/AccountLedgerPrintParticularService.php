...
class AccountLedgerPrintParticularService
{
    ...
    public function purchaseReturnDetails($request, $ledger)
    {
        ...
        $inventoryDetails = '';
        if ($request->inventory_list == 1 && isset($ledger->purchaseReturn->purchaseReturnProducts)) {
                ...
                if (count($description->references) > 0) {
                    ...
                    foreach ($description->references as $reference) {
                        ...
                        if ($reference->payroll) {
                            ...
                            $payroll = '<p class="fw-bold" style="line-height:10px4px">' . __('Payroll') . ' : ' . $reference?->payroll->voucher_no . ' = ' . \App\Utils\Converter::format_in_bdt($reference->amount);
                        }
    ...
    public function saleReturnProductTax($request, $ledger)
    {
        ...
        $inventoryDetails = '';
        if ($request->inventory_list == 1 && isset($salesReturnProduct->salesReturn->saleReturnProducts)) {
    ...
    public function purchaseReturnProductTax($request, $ledger)
    {
        ...
        if ($request->voucher_details == 1) {
            ...
            $voucherDetails .= '<tr>';
            $voucherDetails .= '<td style="line-height:12px !important;padding:0px!important;" class="w-60"><strong>' . __('Total Qty') . '</strong></td>';
            $voucherDetails .= '<td style="line-height:12px !important;padding:0px!important;"> : ' . \App\Utils\Converter::format_in_bdt($purchaseReturnProduct?->purchaseReturn?->total_qty) . '</td>';
            $voucherDetails .= '</tr>';

            $voucherDetails .= '<tr>';
            $voucherDetails .= '<td style="line-height:12px !important;padding:0px!important;" class="w-60"><strong>' . __('Net Total Amount') . '</strong></td>';
            $voucherDetails .= '<td style="line-height:12px !important;padding:0px!important;"> : ' . \App\Utils\Converter::format_in_bdt($purchaseReturnProduct?->purchaseReturn?->net_total_amount) . '</td>';
            $voucherDetails .= '</tr>';

            $voucherDetails .= '<tr>';
            $voucherDetails .= '<td style="line-height:12px !important;padding:0px!important;" class="w-60"><strong>' . __('Return Discount') . '</strong></td>';
            $voucherDetails .= '<td style="line-height:12px !important;padding:0px!important;"> : ' . \App\Utils\Converter::format_in_bdt($purchaseReturnProduct?->purchaseReturn?->return_discount_amount) . '</td>';
            $voucherDetails .= '</tr>';

            $voucherDetails .= '<tr>';
            $voucherDetails .= '<td style="line-height:12px !important;padding:0px!important;" class="w-60"><strong>' . __('Return Tax') . '</strong></td>';
            $voucherDetails .= '<td style="line-height:12px !important;padding:0px!important;"> : ' . \App\Utils\Converter::format_in_bdt($purchaseReturnProduct?->purchaseReturn?->return_tax_amount) . '</td>';
            $voucherDetails .= '</tr>';

            $voucherDetails .= '<tr>';
            $voucherDetails .= '<td style="line-height:12px !important;padding:0px!important;" class="w-60"><strong>' . __('Total Returned Amount') . '</strong></td>';
            $voucherDetails .= '<td style="line-height:12px !important;padding:0px!important;"> : ' . \App\Utils\Converter::format_in_bdt($purchaseReturnProduct?->purchaseReturn?->total_return_amount) . '</td>';
            $voucherDetails .= '</tr>';
            $voucherDetails .= '</table>';
        }

        $inventoryDetails = '';
        if ($request->inventory_list == 1 && isset($purchaseReturnProduct->purchaseReturn->purchaseReturnProducts)) {
