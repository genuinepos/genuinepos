...
class CurrentLiabilitiesCashFlowService
{
    public function currentLiabilities(object $request): object
    {
        $fromDateYmd = '';
        $toDateYmd = '';
        ...
        $mappedArray = [
            'main_group_id' => '',
            'main_group_name' => 'Current Liabilities',
            'sub_group_number' => '',
        ...
        $dutiesAndTaxesAccount = $this->dutiesAndTaxesAccount(request: $request);
        $accountPayable = $this->accountPayable(request: $request);

        $cashIn = 0;
        $cashOut = 0;
        $cashIn += $dutiesAndTaxesAccount->cash_in;
        $cashOut += $dutiesAndTaxesAccount->cash_out;
        $cashIn += $accountPayable->cash_in;
        $cashOut += $accountPayable->cash_out;
    ...
    private function dutiesAndTaxesAccount(object $request): object
    {
        $filteredBranchId = null;
        $filteredChildBranchId = isset($request->child_branch_id)
            && $request->child_branch_id && !empty($request->child_branch_id) ?
                            ...
                            SUM(
                                CASE
                                    WHEN ' . ($__branchId !== null ? 'account_ledgers.branch_id = ' . $__branchId : 'account_ledgers.branch_id IS NULL') . '
                                        ...
                                        AND account_ledgers.is_cash_flow = 1
                                        AND timestamp(account_ledgers.date) < \'' . $toDateYmd . '\'
                                    OR ' . ($__branchId !== null ? 'parentBranch.id = ' . $__branchId : 'parentBranch.id IS NULL') . '
                                    THEN account_ledgers.debit
                                END
                            ), 0
                        ) AS cash_out
    ...
    private function accountPayable(object $request): object
    {
        $filteredBranchId = null;
        $filteredChildBranchId = isset($request->child_branch_id)
            && $request->child_branch_id && !empty($request->child_branch_id) ?
