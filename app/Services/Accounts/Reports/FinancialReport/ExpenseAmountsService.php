...
class ExpenseAmountsService
{
    public function details($request, $accountStartDate)
    {
        $expenseAccounts = $this->expenseAccounts($request, $accountStartDate);

        $totalDebit = 0;
        $totalDebit += $expenseAccounts->closing_balance_side == 'dr' ? $expenseAccounts->closing_balance : 0;
        $totalCredit = 0;
        $totalCredit += $expenseAccounts->closing_balance_side == 'cr' ? $expenseAccounts->closing_balance : 0;
    ...
    private function expenseAccounts($request, $accountStartDate)
    {
        $accountStartDateYmd = '';
        $fromDateYmd = '';
        $toDateYmd = '';
        ...
        $query = DB::table('account_groups')
            ->where('account_groups.main_group_number', 3)
            ->whereIn('account_groups.sub_group_number', [10, 11, 12])
