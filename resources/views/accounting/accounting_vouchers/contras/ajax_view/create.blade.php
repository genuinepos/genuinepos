...
<div class="modal-dialog col-55-modal" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h6 class="modal-title" id="exampleModalLabel">{{ __("Add Contra") }}</h6>
            <a href="#" class="close-btn" data-bs-dismiss="modal" aria-label="Close"><span class="fas fa-times"></span></a>
        </div>

        <div class="modal-body">
            <form id="add_contra_form" action="{{ route('contras.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="action" id="action">
                <input type="hidden" name="print_page_size" id="print_page_size" value="{{ $generalSettings['print_page_size__contra_voucher_page_size'] }}">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="row" style="border-right:1px solid black;">
                            <div class="col-md-12">
                                ...
                                <input required name="date" class="form-control" id="contra_date" data-next="contra_credit_account_id" value="{{ date($generalSettings['business_or_shop__date_format']) }}" placeholder="{{ __("Date") }}" autocomplete="off">
                            ...
                            <div class="col-md-12">
                                ...
                                <input name="transaction_no" class="form-control" id="contra_transaction_no" data-next="contra_cheque_no" placeholder="{{ __("Transaction No") }}">
                            </div>

                            <div class="col-md-12">
                                <label class="fw-bold">{{ __("Cheque No") }}</label>
                                <input name="cheque_no" class="form-control" id="contra_cheque_no" data-next="contra_cheque_serial_no" placeholder="{{ __("Cheque No") }}">
                            </div>

                            <div class="col-md-12">
                                <label class="fw-bold">{{ __("Cheque Serial No") }}</label>
                                <input name="cheque_serial_no" class="form-control" id="contra_cheque_serial_no" data-next="contra_reference" placeholder="{{ __("Cheque Serial No") }}">
                            </div>

                            <div class="col-md-12">
                                <label class="fw-bold">{{ __("Reference") }}</label>
                                <input name="reference" class="form-control" id="contra_reference" data-next="contra_remarks" placeholder="{{ __("reference") }}">
                            </div>

                            <div class="col-md-12">
                                <label class="fw-bold">{{ __("Remarks") }}</label>
                                <input name="remarks" class="form-control" id="contra_remarks" data-next="contra_debit_account_id" placeholder="{{ __("Remarks") }}">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                ...
                                <input type="number" step="any" name="received_amount" class="form-control fw-bold" id="contra_received_amount" data-next="save_and_print" placeholder="{{ __("Received Amount") }}">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group d-flex mt-2 justify-content-end" style="gap: 20px;">
                    <div class="input-group" style="width: max-content; align-items: center; gap: 10px;">
                        ...
                        <select id="select_print_page_size" class="form-control">
                            @foreach (\App\Enums\PrintPageSize::cases() as $item)
                                <option {{ $generalSettings['print_page_size__contra_voucher_page_size'] == $item->value ? 'SELECTED' : '' }} value="{{ $item->value }}">{{ App\Services\PrintPageSizeService::pageSizeName($item->value, false) }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="d-flex justify-content-end">
                        <div class="btn-loading">
                            ...
                            <button type="submit" id="save_and_print" value="save_and_print" class="btn btn-sm btn-success contra_submit_button me-2" value="save_and_print">{{ __("Save & Print") }}</button>
                            <button type="submit" id="save" value="save" class="btn btn-sm btn-success contra_submit_button me-2" value="save">{{ __("Save") }}</button>
...
@include('accounting.accounting_vouchers.contras.ajax_view.js_partials.add_js')
