...
<div class="print_area">
    ...
    <div class="row mt-2">
        <div class="col-12 text-center">
            <h6 style="text-transform:uppercase;"><strong>{{ __('Vat/Tax Report') }}</strong></h6>
        </div>
    </div>
    ...
    <div class="row mt-2">
        ...
        <div class="col-6">
            <p><strong>{{ __('Vat/Tax Ledger A/c') }} : </strong> {{ $filteredTaxAccountName }} </p>
        </div>
    </div>
            ...
            <table class="table report-table table-sm table-bordered print_table">
                <thead>
                    <tr>
                        ...
                        <th class="text-start">{{ __('Vat/Tax Ledger A/c') }}</th>
                        <th class="text-end">{{ __('Amount') }}</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($amounts['inputTaxes'] as $inputTax)
                        <tr>
                            <td class="text-start">{{ $loop->index + 1 }}</td>
                            <td class="text-start">{{ $inputTax->name }}</td>

                            <td class="text-end fw-bold">
                                {{ App\Utils\Converter::format_in_bdt($inputTax->total_input_tax) }}
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    <div class="row mt-1">
        <div class="col-12">
            ...
            <table class="table report-table table-sm table-bordered print_table">
                ...
                <tbody>
                    @foreach ($amounts['outputTaxes'] as $outputTax)
                        <tr>
                            <td class="text-start">{{ $loop->index + 1 }}</td>
                            <td class="text-start">{{ $outputTax->name }}</td>

                            <td class="text-end fw-bold">
                                {{ App\Utils\Converter::format_in_bdt($outputTax->total_output_tax) }}
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    <div class="row">
        <div class="col-6 offset-6">
            <table class="table report-table table-sm table-bordered print_table">
                <thead>
                    <tr>
                        <th class="text-end">{{ __('Total Output Vat/Tax') }} : {{ $generalSettings['business_or_shop__currency_symbol'] }}</th>
                        <td class="text-end">
                            {{ App\Utils\Converter::format_in_bdt($amounts['totalOutputTaxAmount']) }}
                        </td>
                    </tr>

                    <tr>
                        <th class="text-end">{{ __('Total Inpt Vat/Tax') }} : {{ $generalSettings['business_or_shop__currency_symbol'] }}</th>
                        <td class="text-end">
                            ({{ App\Utils\Converter::format_in_bdt($amounts['totalInputTaxAmount']) }})
                        </td>
                    </tr>

                    <tr>
                        <th class="text-end">{{ __('Net Amount') }} : {{ $generalSettings['business_or_shop__currency_symbol'] }}</th>
                        <td class="text-end">
                            {{ App\Utils\Converter::format_in_bdt($amounts['netAmount']) }}
                        </td>
                    </tr>
                </thead>
            </table>
