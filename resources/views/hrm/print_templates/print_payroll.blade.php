                    ...
                    @if ($payroll->branch)
                        ...
                        @if ($payroll?->branch?->parent_branch_id)
                            ...
                            @if ($payroll->branch?->parentBranch?->logo)
                                <img style="height: 60px; width:200px;" src="{{ Storage::disk('s3')->url(tenant('id') . '/' . 'branch_logo/' . $payroll->branch?->parentBranch?->logo) }}">
                            @else
                        ...
                        @else
                            @if ($payroll->branch?->logo)
                                <img style="height: 60px; width:200px;" src="{{ Storage::disk('s3')->url(tenant('id') . '/' . 'branch_logo/' . $payroll->branch?->logo) }}">
                            @else
                ...
                <div class="col-4">
                    @if ($payroll->branch)
                        ...
                        @if ($payroll?->branch?->parent_branch_id)
                            ...
                            @if ($payroll->branch?->parentBranch?->logo)
                                <img style="height: 45px; width:200px;" src="{{ Storage::disk('s3')->url(tenant('id') . '/' . 'branch_logo/' . $payroll->branch?->parentBranch?->logo) }}">
                            @else
                                <span style="font-family: 'Anton', sans-serif;font-size:15px;color:gray;text-transform:uppercase;">{{ $payroll->branch?->parentBranch?->name }}</span>
                            @endif
                        @else
                            @if ($payroll->branch?->logo)
                                <img style="height: 45px; width:200px;" src="{{ Storage::disk('s3')->url(tenant('id') . '/' . 'branch_logo/' . $payroll->branch?->logo) }}">
                            @else
                                <span style="font-family: 'Anton', sans-serif;font-size:15px;color:gray;text-transform:uppercase;">{{ $payroll->branch?->name }}</span>
                            @endif
                        @endif
                    @else
                        @if ($generalSettings['business_or_shop__business_logo'] != null)
                            <img style="height: 45px; width:200px;" src="{{ Storage::disk('s3')->url(tenant('id') . '/' . 'business_logo/' . $generalSettings['business_or_shop__business_logo']) }}" alt="logo" class="logo__img">
                        @else
                            <span style="font-family: 'Anton', sans-serif;font-size:15px;color:gray;text-transform:uppercase;">{{ $generalSettings['business_or_shop__business_name'] }}</span>
                        @endif
                    @endif
