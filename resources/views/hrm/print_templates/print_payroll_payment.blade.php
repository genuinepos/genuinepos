                    ...
                    @if ($payment->branch)
                        ...
                        @if ($payment?->branch?->parent_branch_id)
                            ...
                            @if ($payment->branch?->parentBranch?->logo)
                                <img style="height: 60px; width:200px;" src="{{ Storage::disk('s3')->url(tenant('id') . '/' . 'branch_logo/' . $payment->branch?->parentBranch?->logo) }}">
                            @else
                        ...
                        @else
                            @if ($payment->branch?->logo)
                                <img style="height: 60px; width:200px;" src="{{ Storage::disk('s3')->url(tenant('id') . '/' . 'branch_logo/' . $payment->branch?->logo) }}">
                            @else
                ...
                <div class="col-4">
                    @if ($payment->branch)
                        ...
                        @if ($payment?->branch?->parent_branch_id)
                            ...
                            @if ($payment->branch?->parentBranch?->logo)
                                <img style="height: 40px; width:200px;" src="{{ Storage::disk('s3')->url(tenant('id') . '/' . 'branch_logo/' . $payment->branch?->parentBranch?->logo) }}">
                            @else
                                <span style="font-family: 'Anton', sans-serif;font-size:15px;color:gray;text-transform:uppercase;">{{ $payment->branch?->parentBranch?->name }}</span>
                            @endif
                        @else
                            @if ($payment->branch?->logo)
                                <img style="height: 40px; width:200px;" src="{{ Storage::disk('s3')->url(tenant('id') . '/' . 'branch_logo/' . $payment->branch?->logo) }}">
                            @else
                                <span style="font-family: 'Anton', sans-serif;font-size:15px;color:gray;text-transform:uppercase;">{{ $payment->branch?->name }}</span>
                            @endif
                        @endif
                    @else
                        @if ($generalSettings['business_or_shop__business_logo'] != null)
                            <img style="height: 40px; width:200px;" src="{{ Storage::disk('s3')->url(tenant('id') . '/' . 'business_logo/' . $generalSettings['business_or_shop__business_logo']) }}" alt="logo" class="logo__img">
                        @else
                            <span style="font-family: 'Anton', sans-serif;font-size:15px;color:gray;text-transform:uppercase;">{{ $generalSettings['business_or_shop__business_name'] }}</span>
                        @endif
                    @endif
