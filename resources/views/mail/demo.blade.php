  ...
  <table style="max-width:670px;margin:50px auto 10px;background-color:#fff;padding:50px;-webkit-border-radius:3px;-moz-border-radius:3px;border-radius:3px;-webkit-box-shadow:0 1px 3px rgba(0,0,0,.12),0 1px 2px rgba(0,0,0,.24);-moz-box-shadow:0 1px 3px rgba(0,0,0,.12),0 1px 2px rgba(0,0,0,.24);box-shadow:0 1px 3px rgba(0,0,0,.12),0 1px 2px rgba(0,0,0,.24); border-top: solid 10px green;">
    <thead>
      <tr>
        ...
        <th style="text-align:right;font-weight:400;">{{ date($generalSettings['business']['date_format'] ,strtotime($sale->date)) . ' ' . date($timeFormat, strtotime($sale->time)) }}</th>
      </tr>
    </thead>
    <tbody>
      <tr>
      ...
      <tr>
        ...
        @if ($sale->branch)
            <td style="width:50%;padding:20px;vertical-align:top">
                <h6 style="margin:0 0 10px 0;padding:0;font-size:14px;"><span style="display:block;font-weight:bold;font-size:13px;">{{ $generalSettings['business']['shop_name'] }}</span> </h6>
        ...
        @else
            <td style="width:50%;padding:20px;vertical-align:top">
                <h6 style="margin:0 0 10px 0;padding:0;font-size:14px;"><span style="display:block;font-weight:bold;font-size:13px;">{{ $generalSettings['business']['shop_name'] }}</span> </h6>
                <p style="margin:0 0 10px 0;padding:0;font-size:14px;"><span style="display:block;font-weight:bold;font-size:13px;">{{ $generalSettings['business']['address'] }}</span> </p>
                <p style="margin:0 0 10px 0;padding:0;font-size:14px;"><span style="display:block;font-weight:bold;font-size:13px;">@lang('menu.phone') :</span> {{ $generalSettings['business']['phone'] }}</p>
            </td>
        @endif
      </tr>
      <tr>
      ...
      <tr>
        <td colspan="2" style="font-size:14px;padding:50px 15px 0 15px;">
          <strong style="display:block;margin:0 0 10px 0;">Regards</strong> <br>
            ...
            <br><br>
            <b>@lang('menu.phone'):</b> {{ $generalSettings['business']['phone'] }}<br>
            <b>@lang('menu.email'):</b> {{ $generalSettings['business']['email'] }}
        </td>
      </tr>
    </tfoot>
  </table>
