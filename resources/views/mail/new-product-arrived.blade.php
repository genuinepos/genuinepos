    ...
    <table style="max-width:670px;margin:50px auto 10px;background-color:#fff;padding:50px;-webkit-border-radius:3px;-moz-border-radius:3px;border-radius:3px;-webkit-box-shadow:0 1px 3px rgba(0,0,0,.12),0 1px 2px rgba(0,0,0,.24);-moz-box-shadow:0 1px 3px rgba(0,0,0,.12),0 1px 2px rgba(0,0,0,.24);box-shadow:0 1px 3px rgba(0,0,0,.12),0 1px 2px rgba(0,0,0,.24); border-top: solid 10px green;">
        <thead>
            <tr>
                ...
                <th style="text-align:right;font-weight:400;">{{ __('Date:') }}{{ date($generalSettings['business_or_shop__date_format'], strtotime($addProduct->date)) }}</th>
            </tr>
        </thead>
        <tbody>
            <tr>
            ...
            <p style="font-size:14px;margin:0 0 0 0;">
                ...
                {{ $generalSettings['business_or_shop__currency_symbol'] }} {{ number_format($addProduct->total_payable_amount, 2) }}
            </p>
            <p style="font-size:14px;margin:0 0 0 0;">
                <span style="font-weight:bold;display:inline-block;min-width:146px">@lang('menu.total_paid') </span>
                {{ $generalSettings['business_or_shop__currency_symbol'] }} {{ $addProduct->paid }}
            </p>
            <p style="font-size:14px;margin:0 0 0 0;">
                <span style="font-weight:bold;display:inline-block;min-width:146px">@lang('menu.due') </span>
                {{ $generalSettings['business_or_shop__currency_symbol'] }} {{ $addProduct->due }}
            </p> --}}
                </td>
            </tr>
            <tr>
        ...
        @else
            <td style="width:50%;padding:20px;vertical-align:top">
                ...
                <p style="margin:0 0 10px 0;padding:0;font-size:14px;"><span style="display:block;font-weight:bold;font-size:13px;">@lang('menu.phone') </span> {{ $generalSettings['business_or_shop__phone'] }}</p>
            </td>
        @endif --}}
            </tr>
            <tr>
