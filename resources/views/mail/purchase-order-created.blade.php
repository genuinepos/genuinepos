...
<body style="background-color:#e2e1e0;font-family: Open Sans, sans-serif;font-size:100%;font-weight:400;line-height:1.4;color:#000;">
  <table style="max-width:670px;margin:50px auto 10px;background-color:#fff;padding:50px;-webkit-border-radius:3px;-moz-border-radius:3px;border-radius:3px;-webkit-box-shadow:0 1px 3px rgba(0,0,0,.12),0 1px 2px rgba(0,0,0,.24);-moz-box-shadow:0 1px 3px rgba(0,0,0,.12),0 1px 2px rgba(0,0,0,.24);box-shadow:0 1px 3px rgba(0,0,0,.12),0 1px 2px rgba(0,0,0,.24); border-top: solid 10px green;">
    ...
    <tbody>
      ...
      <tr>
        <td colspan="9" style="border: solid 1px #ddd; padding:10px 20px;">
            <p style="font-size:14px;margin:0 0 6px 0;"> {{ __('P/O ID :') }}{{ $purchase['invoice_id'] }}</p>
      ...
      <tr>
        <td colspan="9" style="font-size:20px;padding:30px 15px 0 14px;">@lang('menu.description')</td>
      </tr>
      <tr>
        ...
        @foreach ($purchase->purchase_order_products as $product)
            <tr>
                ...
                <td style="font-size:11px; text-align:end">{{ $product->pending_quantity }}</td>
                <td style="font-size:11px; text-align:end">{{ $product->received_quantity }}</td>
            </tr>
            @php $index++; @endphp
        @endforeach
        <tr>
        ...
        <tr>
            <th colspan="7" style="font-size:11px; text-align:end">@lang('menu.order_tax') : {{ $generalSettings['business__currency'] }}</th>
            <td colspan="2" style="font-size:11px; text-align:end">
        ...
        <tr>
            <th colspan="7" style="font-size:11px; text-align:end">{{ __('Order Total') }} : {{ $generalSettings['business__currency'] }}</th>
            <td colspan="2" style="font-size:11px; text-align:end">
