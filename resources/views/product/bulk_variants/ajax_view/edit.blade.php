<div class="modal-dialog col-40-modal" role="document">
    <div class="modal-content">
        ...
        <div class="modal-body">
            <form id="edit_bulk_variant_form" action="{{ route('product.bulk.variants.update', $bulkVariant->id) }}" method="POST">
                @csrf
                <div class="form-group row">
                    <div class="col-md-12">
                        <label><b>{{ __("Name") }}</b> <span class="text-danger">*</span></label>
                        <input required type="text" name="name" class="form-control" id="variant_name" data-next="variant_child" value="{{ $bulkVariant->name }}" placeholder="{{ __("Variant Name") }}" />
                ...
                <div class="form-group row">
                    ...
                    <table class="table table-sm w-100">
                        <tbody id="variant_child_fields">
                            @foreach ($bulkVariant->bulkVariantChild as $bulkVariantChild)
                                <tr>
                                    <td style="width: 70%!important; line-height:1!important;padding:3px;vertical-align: bottom;">
                                        <input required type="text" name="variant_child[]" class="form-control" id="variant_child" value="{{ $bulkVariantChild->name }}" placeholder="{{ __("Variant Child") }}" />
                                        <input type="hidden" name="variant_child_ids[]" value="{{ $bulkVariantChild->id }}"/>
                                        @php
                                            $uniqueId = uniqid();
                                        @endphp
                ...
                <div class="form-group row mt-2">
                    <div class="col-md-12 d-flex justify-content-end">
                        <div class="btn-loading">
                            ...
                            <button type="button" id="variant_save_changes" class="btn btn-sm btn-success variant_submit_button">{{ __("Save Changes") }}</button>
                        </div>
                    </div>
                </div>
            </form>
...
<script>
    ...
    $('#edit_bulk_variant_form').on('submit', function(e) {
        e.preventDefault();
    ...
    function addMore() {
        ...
        html += '<input type="hidden" name="variant_child_ids[]"/>';
