...
<script>
                    ...
                    if (!$.isEmptyObject(product.product)) {
                        ...
                        if (product.variants.length == 0) {
                            ...
                            $.each(product.variants, function(key, variant) {
                                ...
                                li += '<li>';
                                li += '<a class="select_variant_product" onclick="selectProduct(this); return false;" data-product_type="variant" data-p_id="' + product.id + '" data-is_manage_stock="' + product.is_manage_stock + '" data-v_id="' + variant.id + '" data-p_name="' + product.name + ' - ' + variant.variant_name + ' (' + variant.variant_code + ')' + '" data-v_name="' + variant.variant_name + '" data-p_tax_ac_id="' + (product.tax_ac_id != null ? product.tax_ac_id : '') + '" data-tax_type="' + product.tax_type + '" data-p_code="' + variant.variant_code + '" data-p_cost_exc_tax="' + variant.variant_cost + '" data-p_cost_inc_tax="' + variant.variant_cost_with_tax + '" href="#"><img style="width:20px; height:20px;" src="' + product.thumbnail_photo + '"> ' + product.thumbnail_photo + '"> ' + product.name + ' - ' + variant.variant_name + ' (' + variant.variant_code + brand + ')' + '</a>';
                                li += '</li>';
                            });
                        ...
                        $('#e_discount_type').val(1);
                        $('#e_tax_ac_id').val(variant.product.tax_id);
                        $('#e_tax_type').val(variant.product.tax_type);
                        $('#e_unit_cost_exc_tax').val(variant.variant_cost);
                        $('#e_unit_cost_inc_tax').val(variant.variant_cost_with_tax);
                        ...
                        if (product.namedProducts.length > 0) {
                            ...
                            $.each(products, function(key, product) {
                                ...
                                if (product.is_variant == 1) {
                                    ...
                                    li += '<li>';
                                    li += '<a class="select_single_product" onclick="selectProduct(this); return false;" data-product_type="single" data-p_id="' + product.id + '" data-v_id="" data-v_name="" data-is_manage_stock="' + product.is_manage_stock + '" data-p_name="' + product.name + ' (' + product.product_code + ')' + '" data-p_code="' + product.product_code + '" data-p_cost_exc_tax="' + product.product_cost + '" data-tax_type="' + product.tax_type + '" data-p_tax_ac_id="' + (product.tax_ac_id != null ? product.tax_ac_id : '') + '" data-p_cost_inc_tax="' + product.product_cost_with_tax + '" href="#"><img style="width:20px; height:20px;" src="' + product.thumbnail_photo + '"> ' + product.thumbnail_photo + '"> ' + product.name + ' (' + product.product_code + brand + ')' + '</a>';
                                    li += '</li>';
                                }
                            });
    ...
    function calculateTotalAmount() {
        ...
        if (accountDefaultBalanceType == 'dr') {
            ...
            currentBalance = parseFloat(closingBalance) + parseFloat(returnedAmount) - parseFloat(receivedAmount);
        } else {
