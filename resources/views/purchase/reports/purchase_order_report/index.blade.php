            ...
            <div class="row">
                <div class="border-class">
                    <div class="main__content">
                        <div class="sec-name">
                            <div class="name-head">
                                <h5>{{ __('Purchase Order Report') }}</h5>
                            </div>
                                    ...
                                    <table class="display data_tbl data__table">
                                        ...
                                        <tfoot>
                                            <tr class="bg-secondary">
                                                <th colspan="4" class="text-end text-white">{{ __('Total') }} : {{ $generalSettings['business__currency_symbol'] }}</th>
                                                <th id="total_po_qty" class="text-white"></th>
                                                <th id="total_received_qty" class="text-white"></th>
                                                <th id="total_pending_qty" class="text-white"></th>
...
@push('scripts')
    ...
    <script>
        var table = $('.data_tbl').DataTable({
            ...
            "ajax": {
                "url": "{{ route('reports.purchase.orders.index') }}",
                "data": function(d) {
            ...
            fnDrawCallback: function() {
                ...
                var total_po_qty = sum_table_col($('.data_tbl'), 'po_qty');
                $('#total_po_qty').text(bdFormat(total_po_qty));

                var total_received_qty = sum_table_col($('.data_tbl'), 'po_received_qty');
                $('#total_received_qty').text(bdFormat(total_received_qty));

                var total_pending_qty = sum_table_col($('.data_tbl'), 'po_pending_qty');
