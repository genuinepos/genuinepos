...
@endpush
@section('title', 'Purchase Ordered Products Report - ')
@section('content')
            ...
            <div class="row">
                <div class="border-class">
                    <div class="main__content">
                        <div class="sec-name">
                            <div class="name-head">
                                <h5>{{ __("Purchase Ordered Products Report") }}</h5>
                            </div>
                                    ...
                                    <table class="display data_tbl data__table">
                                        ...
                                        <tfoot>
                                            <tr class="bg-secondary">
                                                ...
                                                <th class="text-start text-white">(<span id="total_ordered_qty"></span>)</th>
                                                <th class="text-start text-white">(<span id="total_received_qty"></span>)</th>
                                                <th class="text-start text-white">(<span id="total_pending_qty"></span>)</th>
                                                <th class="text-start text-white">---</th>
                                                <th class="text-start text-white">---</th>
                                                <th class="text-start text-white">---</th>
                                                <th class="text-start text-white">---</th>
                                                <th class="text-start text-white" id="total_linetotal"></th>
                                            </tr>
                                        </tfoot>
                                    </table>
                                </div>
...
<script>
    var table = $('.data_tbl').DataTable({
        ...
        "ajax": {
            "url": "{{ route('reports.purchase.ordered.products.index') }}",
            "data": function(d) {
        ...
        columns: [
            ...
            {data: 'ordered_quantity', name: 'ordered_quantity', name: 'parentBranch.name', className: 'text-end fw-bold'},
            {data: 'received_quantity', name: 'received_quantity', name: 'parentBranch.name', className: 'text-end fw-bold'},
            {data: 'pending_quantity', name: 'pending_quantity', name: 'parentBranch.name', className: 'text-end fw-bold'},
            {data: 'unit_cost_exc_tax', name: 'purchase_products.unit_cost_exc_tax', className: 'text-end fw-bold'},
            {data: 'unit_discount_amount', name: 'purchase_products.unit_discount_amount', className: 'text-end fw-bold'},
            {data: 'unit_tax_amount', name: 'purchase_products.unit_tax_amount', className: 'text-end fw-bold'},
            {data: 'net_unit_cost', name: 'purchase_products.net_unit_cost', className: 'text-end fw-bold'},
            {data: 'line_total', name: 'purchase_products.line_total', className: 'text-end fw-bold'},
        ],
        fnDrawCallback: function() {
            var total_ordered_qty = sum_table_col($('.data_tbl'), 'ordered_quantity');
            $('#total_ordered_qty').text(bdFormat(total_ordered_qty));

            var total_received_qty = sum_table_col($('.data_tbl'), 'received_quantity');
            $('#total_received_qty').text(bdFormat(total_received_qty));

            var total_pending_qty = sum_table_col($('.data_tbl'), 'pending_quantity');
            $('#total_pending_qty').text(bdFormat(total_pending_qty));

            var line_total = sum_table_col($('.data_tbl'), 'line_total');
            $('#total_linetotal').text(bdFormat(line_total));
            $('.data_preloader').hide();
        },
    });
    ...
    $(document).on('click', '#print_report', function (e) {
        e.preventDefault();
        var url = "{{ route('reports.purchase.ordered.products.print') }}";
        ...
        $.ajax({
            url:url,
            type:'get',
            data: {search_product, branch_id, branch_name, product_id, supplier_account_id, supplier_name, variant_id, from_date, to_date},
            success:function(data){
                $(data).printThis({
                    debug: false,
                    importCSS: true,
