...
@section('content')
    <div class="body-woaper">
        <div class="container-fluid">
            <form id="edit_purchase_return_form" action="{{ route('purchases.returns.supplier.return.update', $return->id) }}" enctype="multipart/form-data" method="POST">
                @csrf
                <section class="mt-5">
                    <div class="container-fluid">
                        <div class="row">
                                                        ...
                                                        <table class="display data__table table-striped">
                                                            ...
                                                            <tbody id="purchase_return_list">
                                                                ...
                                                                @foreach ($return->purchase_return_products as $return_product)
                                                                   ...
                                                                   <tr>
                                                                        <td class="text">
                                                                            <span class="product_name">{{ $return_product->product->name }}</span>
                                                                            <span class="product_variant">
                                                                                {{ $return_product->variant ? $return_product->variant->variant_name : '' }}
                                                                            </span>
                                                                            <input value="{{ $return_product->product_id }}" type="hidden" class="productId-{{ $return_product->product_id }}" id="product_id" name="product_ids[]">
                                                                            <input value="{{ $return_product->product_variant_id ? $return_product->product_variant_id : 'noid' }}" type="hidden" class="variantId-{{ $return_product->product_variant_id }}" id="variant_id" name="variant_ids[]">
                                                                            <input type="hidden" id="previous_quantity" value="{{ $return_product->return_qty }}">
...
@push('scripts')
    ...
    <script>
        ...
        $(document).on('input', '#return_quantity', function(){
            ...
            if(parseInt(qty) > parseInt(__qty_limit)){

                alert('Quantity Limit Is - '+qty_limit+' '+unit);

                $(this).val(__qty_limit);
