@if (isset($mb_product))
    ...
    <tr>
        <td colspan="2" class="text-start">
            ...
            <span class="product_name">{{ $mb_product->name }}</span>
            ...
            <input name="unit_costs_inc_tax[]" type="hidden" id="unit_cost_inc_tax" value="{{ $mb_product->product_cost_with_tax }}">
            <input value="0.00" type="hidden" id="previous_quantity">
            <input type="hidden" id="qty_limit" value="{{ $mb_product->mb_stock }}">
        </td>
        ...
        <td class="text">
            <b><span class="span_unit">{{ $mb_product->unit->name }}</span></b>
            <input  name="units[]" type="hidden" id="unit" value="{{ $mb_product->unit->name }}">
        </td>

        <td>
            <input readonly name="unit_prices_exc_tax[]" type="hidden"  id="unit_price_exc_tax" value="{{ $mb_product->product_price }}">
            @php
                $unitPriceIncTax = $mb_product->product_price / 100 * $tax_percent + $mb_product->product_price; 
            @endphp
...
@else
    ...
    <tr>
        <td colspan="2" class="text-start">
            ...
            <input type="{{ ($product->product->is_show_emi_on_pos == 1 ? 'text' : 'hidden') }}" name="descriptions[]" class="form-control scanable mb-1" placeholder="IMEI, Serial number or other info.">
