@if (isset($mbProduct))
    @php
    $tax_percent = $mbProduct->tax_id ? $mbProduct->tax->tax_percent : 0;
    $tax_amount = $mbProduct->tax ? ($mbProduct->product_price / 100) * $mbProduct->tax->tax_percent : 0;  
    @endphp
    <tr>
        <td class="serial text-start">1</td>
        <td class="text-start">
            <a class="product-name text-info" id="edit_product" href="#">{{ $mbProduct->name }}</a> <br/>
            <input type="{{ ($mbProduct->is_show_emi_on_pos == 1 ? 'text' : 'hidden') }}" name="descriptions[]" class="form-control description_input scanable" placeholder="IMEI, Serial number or other info."> 
            <input value="{{ $mbProduct->id }}" type="hidden" class="productId-{{ $mbProduct->id }}" id="product_id" name="product_ids[]">
            ...
            <input type="hidden" id="tax_type" value="{{ $mbProduct->tax_type }}">
            ...
            <input name="unit_costs_inc_tax[]" type="hidden" id="unit_cost_inc_tax" value="{{ $mbProduct->product_cost_with_tax }}">
            <input type="hidden" id="previous_qty" value="0.00">
            <input type="hidden" id="qty_limit" value="{{ $mbProduct->mb_stock }}">
            <input class="index-{{ $mbProduct->product_code }}" type="hidden" id="index">
        </td>
        ...
        <td>
            <b><span class="span_unit">{{ $mbProduct->unit->name }}</span></b>
            <input name="units[]" type="hidden" id="unit" value="{{ $mbProduct->unit->name }}">
        </td>

        <td>
            <input name="unit_prices_exc_tax[]" type="hidden" value="{{ bcadd($mbProduct->product_price, 0, 2) }}" id="unit_price_exc_tax">
            @php
                $unitPriceIncTax = ($mbProduct->product_price / 100 * $tax_percent) + $mbProduct->product_price;
            @endphp
