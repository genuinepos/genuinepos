<div class="modal-dialog col-40-modal" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h6 class="modal-title" id="exampleModalLabel">{{ __('Add Currency') }}</h6>
            <a href="#" class="close-btn" data-bs-dismiss="modal" aria-label="Close"><span class="fas fa-times"></span></a>
        </div>
        <div class="modal-body">
            <form id="add_currency_form" action="{{ route('currencies.store') }}" method="post">
                @csrf
                <div class="form-group">
                    <label><b>{{ __('Country Name') }}</b> <span class="text-danger">*</span></label>
                    <input required type="text" name="country" class="form-control" data-next="currency" id="country" placeholder="{{ __('Country Name') }}" />
                    <span class="error error_currency_country"></span>
                </div>

                <div class="form-group mt-1">
                    <label><b>{{ __('Currency Name') }}</b> <span class="text-danger">*</span></label>
                    <input required type="text" name="currency" class="form-control" data-next="code" id="currency" placeholder="{{ __('Currency Name') }}" />
                    <span class="error error_currency_currency"></span>
                </div>

                <div class="form-group mt-1">
                    <label><b>{{ __('Currency Code') }}</b> <span class="text-danger">*</span></label>
                    <input required type="text" name="code" class="form-control" data-next="symbol" id="code" placeholder="{{ __('Currency Code') }}" />
                    <span class="error error_currency_code"></span>
                </div>

                <div class="form-group mt-1">
                    <label><b>{{ __('Currency Symbol') }}</b> <span class="text-danger">*</span></label>
                    <input required type="text" name="symbol" class="form-control" data-next="currency_rate" id="symbol" placeholder="{{ __('Currency Symbol') }}" />
                ...
                <div id="currency_rate_fields">
                    <div class="form-group mt-2 row g-2">
                        ...
                        <div class="col-md-4">
                            <input type="text" name="currency_rate" class="form-control fw-bold" id="currency_rate" data-next="currency_save" placeholder="{{ __('0.00') }}" />
                ...
                <div class="form-group row mt-2">
                    <div class="col-md-12 d-flex justify-content-end">
                        <div class="btn-loading">
                            ...
                            <button type="submit" id="currency_save" class="btn btn-sm btn-success currency_submit_button">{{ __('Save') }}</button>
                        </div>
                    </div>
                </div>
            </form>
...
<script>
    ...
    $('#add_currency_form').on('submit', function(e) {
        e.preventDefault();
