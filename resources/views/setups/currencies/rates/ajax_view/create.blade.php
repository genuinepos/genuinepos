<div class="modal-dialog col-40-modal" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h6 class="modal-title" id="exampleModalLabel">{{ __('Add Currency Rate') }} - (<strong>{{ $currency->country . '-' . $currency->currency . '-' . $currency->code . '-' . $currency->symbol }}</strong>)</h6>
            <a href="#" class="close-btn" data-bs-dismiss="modal" aria-label="Close"><span class="fas fa-times"></span></a>
        </div>
        <div class="modal-body">
            <form id="add_currency_rate_form" action="{{ route('currencies.rates.store', $currency->id) }}" method="post">
                @csrf
                <div class="form-group">
                    <label><b>{{ __('Date') }}</b> <span class="text-danger">*</span></label>
                    <input required type="text" name="date" class="form-control" id="currency_rate_date" data-next="currency_rate_rate" value="{{ date($generalSettings['business_or_shop__date_format']) }}" placeholder="{{ __('Date') }}" autocomplete="off" />
                ...
                <div id="currency_rate_fields">
                    ...
                    <div class="form-group mt-1 row g-2">
                        ...
                        <div class="col-md-3">
                            <input required type="text" name="rate" class="form-control fw-bold" id="currency_rate_rate" data-next="currency_rate_save" placeholder="{{ __('0.00') }}" />
                ...
                <div class="form-group row mt-2">
                    <div class="col-md-12 d-flex justify-content-end">
                        <div class="btn-loading">
                            ...
                            <button type="submit" id="currency_rate_save" class="btn btn-sm btn-success currency_rate_submit_button">{{ __('Save') }}</button>
                        </div>
                    </div>
                </div>
            </form>
...
<script>
    ...
    $('#add_currency_rate_form').on('submit', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (typeof currencyRatesTable !== 'undefined') {
                    console.log('currencyRatesTable');
                    currencyRatesTable.ajax.reload(null, false);
                }

                if (typeof currenciesTable !== 'undefined') {

                    console.log('currenciesTable');
