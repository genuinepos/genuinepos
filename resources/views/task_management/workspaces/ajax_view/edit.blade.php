<div class="modal-dialog col-55-modal" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h6 class="modal-title">{{ __("Edit Project") }}</h6>
            <a href="" class="close-btn" data-bs-dismiss="modal" aria-label="Close"><span class="fas fa-times"></span></a>
        </div>
        <div class="modal-body">
            <form id="edit_workspace_form" action="{{ route('workspaces.update', $workspace->id) }}" method="post">
                @csrf
                <div class="form-group row">
                    <div class="col-md-6">
                        <label><b>{{ __("Name") }}</b></label>
                        <input required type="text" name="name" class="form-control" id="workspace_name" data-next="workspace_user_ids"  placeholder="{{ __("Project Name") }}" value="{{ $workspace->name }}">
                    </div>
                            ...
                            @foreach ($users as $user)
                                <option
                                    @foreach ($workspace->users as $workspaceUser)
                                        @selected($workspaceUser->user_id == $user->id)
                                    @endforeach
                    ...
                    <div class="col-md-6">
                        ...
                        <select required name="priority" class="form-control" id="workspace_priority" data-next="workspace_status">
                            ...
                            @foreach (\App\Enums\TaskPriority::cases() as $item)
                                <option @selected($workspace->priority == $item->value) value="{{ $item->value }}">{{ $item->value }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select required name="status" class="form-control" id="workspace_status" data-next="workspace_start_date">
                            ...
                            @foreach (\App\Enums\TaskStatus::cases() as $item)
                                <option @selected($workspace->status == $item->value) value="{{ $item->value }}">{{ $item->value }}</option>
                            @endforeach
                        </select>
                ...
                <div class="form-group row mt-1">
                    <div class="col-md-6">
                        ...
                        <input required type="text" name="start_date" class="form-control" id="workspace_start_date" data-next="workspace_end_date" value="{{ date($generalSettings['business_or_shop__date_format'], strtotime($workspace->start_date)) }}" placeholder="{{ __("Project Start Date") }}">
                        <span class="error error_workspace_start_date"></span>
                    </div>

                    <div class="col-md-6">
                        <label><b>{{ __("End Date") }}</b> <span class="text-danger"> *</span></label>
                        <input required type="text" name="end_date" class="form-control" id="workspace_end_date" data-next="workspace_description" value="{{ date($generalSettings['business_or_shop__date_format'], strtotime($workspace->end_date)) }}" placeholder="{{ __("Project End Date") }}">
                ...
                <div class="form-group row mt-1">
                    <div class="col-md-12">
                        ...
                        <input name="description" class="form-control" id="workspace_description" data-next="workspace_estimated_hours" value="{{ $workspace->description }}" placeholder="{{ __("Project Description") }}">
                    </div>
                </div>
                ...
                <div class="form-group row mt-1">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="estimated_hours" class="form-control" id="workspace_estimated_hours" data-next="workspace_save_changes" placeholder="{{ __('Estimated Hour') }}" value="{{ $workspace->estimated_hours }}">
                    </div>
                </div>
                ...
                <div class="form-group row mt-2">
                    <div class="col-md-12 d-flex justify-content-end">
                        <div class="btn-loading">
                            ...
                            <button type="submit" id="workspace_save_changes" class="btn btn-sm btn-success workspace_submit_button">{{ __("Save") }}</button>
                        </div>
                    </div>
                </div>
            </form>
...
<script>
    ...
    $('#edit_workspace_form').on('submit',function(e) {
        e.preventDefault();
