...
@endpush
@section('title', 'Edit Transfer Stock (Shop/Business To Warehouse)')
@section('content')
    <div class="body-woaper">
        <div class="main__content">
            <div class="sec-name">
                <div class="name-head">
                    ...
                    <h6>{{ __("Edit Tranfer Stock (Shop/Business To Warehouse)") }}</h6>
                </div>
        ...
        <div class="p-1">
            <form id="edit_transfer_branch_warehouse_form" action="{{ route('transfer.stock.branch.to.warehouse.update', $transferStock->id) }}" method="POST">
                @csrf
                <section>
                                        ...
                                        <div class="col-7">
                                            <select name="receiver_warehouse_id" class="form-control" id="receiver_warehouse_id" data-next="date" autofocus>
                                                ...
                                                @foreach ($warehouses as $w)
                                                    <option {{ $transferStock->receiver_warehouse_id == $w->id ? 'SELECTED' : '' }} value="{{ $w->id }}">{{ $w->warehouse_name.'/'.$w->warehouse_code }}</option>
                                                @endforeach
                                            </select>
...
@push('scripts')
    ...
    <script>
        ...
        $('#add_item').on('click', function(e) {
            ...
            var sender_branch_id = "{{ $tranferStock->branch_id }}";
            ...
            if (e_variant_id != 'noid') {
                ...
                var url = "{{ route('general.product.search.variant.product.stock', [':e_product_id', ':e_variant_id', ':sender_branch_id']) }}";
                route = url.replace(':e_product_id', e_product_id);
                route = route.replace(':e_variant_id', e_variant_id);
                route = route.replace(':sender_branch_id', sender_branch_id);
            } else {

                var url = "{{ route('general.product.search.single.product.stock', [':e_product_id', ':sender_branch_id']) }}";
                route = url.replace(':e_product_id', e_product_id);
                route = route.replace(':sender_branch_id', sender_branch_id);
            }
        ...
        $('#edit_transfer_branch_warehouse_form').on('submit', function(e){
            e.preventDefault();
