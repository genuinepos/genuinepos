...
Route::get('/test', function () {
    ...
    foreach ($suppliers as $s) {
        ...
        $supplierLedger = SupplierLedger::where('voucher_type', 0)->where('supplier_id', $s->id)->first();

        if (!$supplierLedger) {

            $addSupplierLedger = new SupplierLedger();
            $addSupplierLedger->supplier_id = $s->id;
            $addSupplierLedger->voucher_type = 0;
            $addSupplierLedger->amount_type = 'credit';
            $addSupplierLedger->amount = 0;
            $addSupplierLedger->credit = 0;
            $addSupplierLedger->report_date = $s->created_at;
            $addSupplierLedger->save();
        }
    }

    // Supplier customer ledgers
    $customerLedgers = CustomerLedger::with('customer', 'sale', 'sale_payment', 'customer_payment')->get();
    ...
    foreach ($customerLedgers as $ledger) {
        ...
        if (!$ledger->sale_id && !$ledger->sale_payment_id && !$ledger->customer_payment_id) {
            $ledger->voucher_type = 0;
            $ledger->debit = $ledger->amount;
            $ledger->amount_type = 'debit';
            $ledger->report_date = (new Carbon($ledger->customer->created_at))->subHours(22)->format('Y-m-d H:i:s');
        }
    ...
    $supplierLedgers = SupplierLedger::with('supplier', 'purchase', 'purchase_payment', 'supplier_payment')->get();

    foreach ($supplierLedgers as $ledger) {
        $ledger->credit = 0;
        $ledger->debit = 0;

        if (!$ledger->purchase_id && !$ledger->purchase_payment_id && !$ledger->supplier_payment_id) {
            $ledger->voucher_type = 0;
            $ledger->credit = $ledger->amount;
            $ledger->amount_type = 'credit';
            $ledger->report_date = (new Carbon($ledger->supplier->created_at))->subHours(22)->format('Y-m-d H:i:s');
        }

        if ($ledger->purchase_id) {

            $ledger->voucher_type = 1;
            $ledger->credit = $ledger->purchase->total_purchase_amount;
            ...
            $ledger->voucher_type = 3;
            $ledger->debit = $ledger->purchase_payment->paid_amount;
            ...
            $ledger->voucher_type = 5;
            $ledger->debit = $ledger->supplier_payment->paid_amount;
    ...
    // foreach (App\Utils\AccountUtil::creatableDefaultAccount() as $account_type => $account_array) {
    ...
    //             'opening_balance' => 0,
    //             'balance' => 0,
    //             (new App\Utils\AccountUtil)->accountBalanceType($account_type) => 0,
    ...
    //         $accountLedger->{(new App\Utils\AccountUtil)->accountBalanceType($account_type)} = 0;
    //         $accountLedger->amount_type = (new App\Utils\AccountUtil)->accountBalanceType($account_type);
    ...
    foreach ($purchases as $purchase) {
        ...
        $purchase->purchase_account_id = $purchaseAccount->id;
        $purchase->save();
    }
    ...
    foreach ($purchases as $purchase) {
        ...
        $accountLedger = new AccountLedger();
        $accountLedger->purchase_id = $purchase->id;
        $accountLedger->date = $purchase->report_date;
        $accountLedger->account_id = $purchaseAccount->id;
        $accountLedger->voucher_type = 3;
        $accountLedger->debit = $purchase->total_purchase_amount;
        $accountLedger->amount_type = 'debit';
        $accountLedger->branch_id = auth()->user()->branch_id;
        $accountLedger->running_balance = (new App\Utils\AccountUtil)->adjustAccountBalance('debit', $purchaseAccount->id);
        $accountLedger->save();
    }
    ...
    foreach ($purchasePayments as $purchasePayment) {
        ...
        $accountLedger = new AccountLedger();
        $accountLedger->purchase_payment_id = $purchasePayment->id;
        $accountLedger->date = $purchasePayment->report_date;
        $accountLedger->account_id = $cashAccount->id;
        $accountLedger->voucher_type = 11;
        $accountLedger->credit = $purchasePayment->paid_amount;
        $accountLedger->amount_type = 'credit';
        $accountLedger->branch_id = auth()->user()->branch_id;
        $accountLedger->running_balance = (new App\Utils\AccountUtil)->adjustAccountBalance('debit', $cashAccount->id);
        $accountLedger->save();
    }
    ...
    foreach ($supplierPayments as $supplierPayment) {
        ...
        $accountLedger = new AccountLedger();
        $accountLedger->supplier_payment_id = $supplierPayment->id;
        $accountLedger->date = $supplierPayment->report_date;
        $accountLedger->account_id = $cashAccount->id;
        $accountLedger->voucher_type = 19;
        $accountLedger->credit = $supplierPayment->paid_amount;
        $accountLedger->amount_type = 'credit';
        $accountLedger->branch_id = auth()->user()->branch_id;
        $accountLedger->running_balance = (new App\Utils\AccountUtil)->adjustAccountBalance('debit', $cashAccount->id);
        $accountLedger->save();
    ...
    foreach ($sales as $sale) {
        ...
        $sale->sale_account_id = $saleAccount->id;
        $sale->save();
    }
    ...
    foreach ($sales as $sale) {
        ...
        $accountLedger = new AccountLedger();
        $accountLedger->sale_id = $sale->id;
        $accountLedger->date = $sale->report_date;
        $accountLedger->account_id = $saleAccount->id;
        $accountLedger->voucher_type = 1;
        $accountLedger->credit = $sale->total_payable_amount;
        $accountLedger->amount_type = 'credit';
        $accountLedger->branch_id = auth()->user()->branch_id;
        $accountLedger->running_balance = (new App\Utils\AccountUtil)->adjustAccountBalance('credit', $saleAccount->id);
        $accountLedger->save();
    }
    ...
    foreach ($salePayments as $salePayment) {
        ...
        $accountLedger = new AccountLedger();
        $accountLedger->sale_payment_id = $salePayment->id;
        $accountLedger->date = $salePayment->report_date;
        $accountLedger->account_id = $cashAccount->id;
        $accountLedger->voucher_type = 10;
        $accountLedger->debit = $salePayment->paid_amount;
        $accountLedger->amount_type = 'debit';
        $accountLedger->branch_id = auth()->user()->branch_id;
        $accountLedger->running_balance = (new App\Utils\AccountUtil)->adjustAccountBalance('debit', $cashAccount->id);
        $accountLedger->save();
    ...
    foreach ($customerPayments as $customerPayment) {
        ...
        $accountLedger = new AccountLedger();
        $accountLedger->customer_payment_id = $customerPayment->id;
        $accountLedger->date = $customerPayment->report_date;
        $accountLedger->account_id = $cashAccount->id;
        $accountLedger->voucher_type = 18;
        $accountLedger->debit = $customerPayment->paid_amount;
        $accountLedger->amount_type = 'debit';
        $accountLedger->branch_id = auth()->user()->branch_id;
        $accountLedger->running_balance = (new App\Utils\AccountUtil)->adjustAccountBalance('debit', $cashAccount->id);
        $accountLedger->save();
    ...
    foreach ($expenses as $expense) {
        ...
        $accountLedger->date = $expense->report_date;
        $accountLedger->account_id = $expenseAccount->id;
        $accountLedger->voucher_type = 5;
        $accountLedger->debit = $expense->total_amount;
        $accountLedger->amount_type = 'debit';
        $accountLedger->branch_id = auth()->user()->branch_id;
        $accountLedger->running_balance = (new App\Utils\AccountUtil)->adjustAccountBalance('debit', $expenseAccount->id);
        $accountLedger->save();
    }
    ...
    foreach ($expensePayments as $expensePayment) {
        ...
        $accountLedger = new AccountLedger();
        $accountLedger->expense_payment_id = $expensePayment->id;
        $accountLedger->date = $expensePayment->report_date;
        $accountLedger->account_id = $cashAccount->id;
        $accountLedger->voucher_type = 9;
        $accountLedger->credit = $expensePayment->paid_amount;
        $accountLedger->amount_type = 'credit';
        $accountLedger->branch_id = auth()->user()->branch_id;
        $accountLedger->running_balance = (new App\Utils\AccountUtil)->adjustAccountBalance('debit', $cashAccount->id);
        $accountLedger->save();
    ...
    foreach ($getLoans as $getLoan) {
        ...
        $getLoan->loan_account_id = $loanLiabilityAccount->id;
        $getLoan->save();
    }
    ...
    foreach ($getLoans as $getLoan) {
        ...
        $accountLedger = new AccountLedger();
        $accountLedger->loan_id = $getLoan->id;
        $accountLedger->date = $getLoan->report_date;
        $accountLedger->account_id = $loanLiabilityAccount->id;
        $accountLedger->voucher_type = 13;
        $accountLedger->credit = $getLoan->loan_amount;
        $accountLedger->amount_type = 'credit';
        $accountLedger->branch_id = auth()->user()->branch_id;
        $accountLedger->running_balance = (new App\Utils\AccountUtil)->adjustAccountBalance('credit', $loanLiabilityAccount->id);
        $accountLedger->save();
    }
    ...
    foreach ($getLoans as $getLoan) {
        ...
        $accountLedger->account_id = $cashAccount->id;
        $accountLedger->voucher_type = 13;
        $accountLedger->debit = $getLoan->loan_amount;
        $accountLedger->amount_type = 'debit';
        $accountLedger->branch_id = auth()->user()->branch_id;
        $accountLedger->running_balance = (new App\Utils\AccountUtil)->adjustAccountBalance('credit', $cashAccount->id);
        $accountLedger->save();
    }

    foreach ($getLoanPayments as $getLoanPayment) {

        $accountLedger = new AccountLedger();
        $accountLedger->loan_payment_id = $getLoanPayment->id;
        $accountLedger->date = $getLoanPayment->report_date;
        $accountLedger->account_id = $cashAccount->id;
        $accountLedger->voucher_type = 15;
        $accountLedger->credit = $getLoanPayment->paid_amount;
        $accountLedger->amount_type = 'credit';
        $accountLedger->branch_id = auth()->user()->branch_id;
        $accountLedger->running_balance = (new App\Utils\AccountUtil)->adjustAccountBalance('debit', $cashAccount->id);
        $accountLedger->save();
    ...
    foreach ($payLoans as $payLoan) {
        ...
        $payLoan->loan_account_id = $loanAdvanceAccount->id;
        $payLoan->save();
    }
    ...
    foreach ($payLoans as $payLoan) {
        ...
        $accountLedger = new AccountLedger();
        $accountLedger->loan_id = $payLoan->id;
        $accountLedger->date = $payLoan->report_date;
        $accountLedger->account_id =  $loanAdvanceAccount->id;
        $accountLedger->voucher_type = 14;
        $accountLedger->debit = $payLoan->loan_amount;
        $accountLedger->amount_type = 'debit';
        $accountLedger->branch_id = auth()->user()->branch_id;
        $accountLedger->running_balance = (new App\Utils\AccountUtil)->adjustAccountBalance('debit', $loanAdvanceAccount->id);
        $accountLedger->save();
    }
    ...
    foreach ($payLoans as $payLoan) {
        ...
        $accountLedger->account_id = $cashAccount->id;
        $accountLedger->voucher_type = 14;
        $accountLedger->credit = $payLoan->loan_amount;
        $accountLedger->amount_type = 'credit';
        $accountLedger->branch_id = auth()->user()->branch_id;
        $accountLedger->running_balance = (new App\Utils\AccountUtil)->adjustAccountBalance('debit', $cashAccount->id);
        $accountLedger->save();
    ...
    foreach ($payLoanPayments as $payLoanPayment) {
        ...
        $accountLedger = new AccountLedger();
        $accountLedger->loan_payment_id = $payLoanPayment->id;
        $accountLedger->date = $payLoanPayment->report_date;
        $accountLedger->account_id = $cashAccount->id;
        $accountLedger->voucher_type = 16;
        $accountLedger->debit = $payLoanPayment->paid_amount;
        $accountLedger->amount_type = 'debit';
        $accountLedger->branch_id = auth()->user()->branch_id;
        $accountLedger->running_balance = (new App\Utils\AccountUtil)->adjustAccountBalance('debit', $cashAccount->id);
        $accountLedger->save();
